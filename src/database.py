...
class Database:
    def __init__(self, db_path: str = None):
        ...
        # Simple cache for guild config: guild_id -> dict
        # Negative caching (storing None) is useful if configs are rarely set but often checked
        self._config_cache = {}
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                ...
                async with db.execute(
                    ...
                    # Convert to dict to make it compatible and cacheable (Row objects might be bound to connection)
                    # And to safely use it after connection closes
                    if row:
